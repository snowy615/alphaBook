...
(function () {
  ...
  function initLobbyPage() {
    ...
    $("#createBtn")?.addEventListener("click", async (e) => {
      ...
      try {
        ...
        const res = await postJSON("/swe-prep/create", { name });
        window.location.href = `/swe-prep/run/${res.run_id}`;
      } catch (err) {
    ...
    $("#joinBtn")?.addEventListener("click", async (e) => {
      ...
      e.target.disabled = true;
      try {
        const res = await postJSON("/swe-prep/join", { join_code: code });
        window.location.href = `/swe-prep/run/${res.run_id}`;
      } catch (err) {
        showMsg(msg, err.message, "error");
        e.target.disabled = false;
    ...
    async function loadOpenRuns() {
      ...
      if (!box) return;
      try {
        const { runs } = await api("/swe-prep/open");
        ...
        box.innerHTML = runs.map((r) => `
          <a class="msp-run-row" href="/swe-prep/run/${encodeURIComponent(r.run_id)}">
  ...
  function initRunPage(runId) {
    ...
    async function loadEditor() {
      ...
      try {
        const [tpl, mine] = await Promise.all([
          api("/swe-prep/starter"),
          api(`/swe-prep/run/${runId}/strategy`),
        ]);
        starter = tpl.code;
    ...
    $("#checkBtn")?.addEventListener("click", async (e) => {
      e.target.disabled = true;
      try {
        const res = await postJSON("/swe-prep/check", { code: editor.value });
    ...
    $("#submitBtn")?.addEventListener("click", async (e) => {
      e.target.disabled = true;
      try {
        await postJSON(`/swe-prep/run/${runId}/strategy`, { code: editor.value });
    ...
    $("#startBtn")?.addEventListener("click", async (e) => {
      e.target.disabled = true;
      try {
        await postJSON(`/swe-prep/run/${runId}/start`, {});
        poll();
      } catch (err) {
        showMsg(msg, err.message, "error");
      } finally {
    ...
    $("#stopBtn")?.addEventListener("click", async (e) => {
      ...
      e.target.disabled = true;
      try {
        await postJSON(`/swe-prep/run/${runId}/stop`, {});
        poll();
      } catch (err) {
        showMsg(msg, err.message, "error");
        e.target.disabled = false;
    ...
    async function poll() {
      let state;
      try {
        state = await api(`/swe-prep/run/${runId}/state`);
      } catch (err) {
        showMsg(msg, err.message, "error");
        schedule("lobby");
        return;
