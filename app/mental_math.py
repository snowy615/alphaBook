...
async def finish_player(game_id: str, user: User = Depends(current_user)):
    ...
    if all_finished and game_data.get("status") != "finished":
        coaching = await _record_scores(game_id, game_data, results)
        for pid, note in coaching.items():
            if pid in results:
                results[pid]["feedback"] = note
        update_data["results"] = results
...
async def _record_scores(game_id: str, game_data: dict, results: dict) -> dict:
    """Rate each player and write them a coaching note. Returns feedback by user."""
    questions = game_data.get("questions", [])
    ...
    for p in game_data.get("players", []):
        ...
        coaching = fb.analyse("mental_math", {
            "questions": questions,
