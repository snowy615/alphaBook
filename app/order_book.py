...
getcontext().prec = 28  # safe decimal math
...
class OrderBook:
    def __init__(self):
        self.bids: Dict[Decimal, Deque[Order]] = {}  # price -> FIFO queue
    ...
    def add(self, order: Order) -> List[Tuple[Decimal, Decimal, str]]:
        """
        Add a LIMIT order; match against opposite side with price-time priority.
        Returns list of trades: (price, qty, aggressor_side)
        """
        trades: List[Tuple[Decimal, Decimal, str]] = []

        if order.side == "BUY":
            # Cross with best asks at or below price
            while order.qty > 0 and self._best_ask() is not None and self._best_ask() <= order.price:
                ...
                head.qty -= fill
                order.qty -= fill
                trades.append((best_px, fill, "BUY"))
                if head.qty == 0:
            ...
            while order.qty > 0 and self._best_bid() is not None and self._best_bid() >= order.price:
                ...
                head.qty -= fill
                order.qty -= fill
                trades.append((best_px, fill, "SELL"))
                if head.qty == 0:
