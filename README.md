...
- **Market Simulation Coding** — write a trading bot, run it on your own machine, and trade four synthetic items over a rate-limited order API against everyone else's bots for ten minutes; no code runs on the server, 1000-lot position limits, P&L marked at hidden fair value. Everything your bot earns becomes credits on a shared map, where you build an empire and fight over territory
...
**Market Simulation Coding** (`algo_engine.py` + `market_sim_py.py` + `world.py`) is client-side: players run their bot on their own machine and reach the market only through an authenticated, rate-limited order gateway (`algo_ratelimit.py` caps it at ~10 orders/sec). No player code runs on the server, so there is nothing to sandbox — the engine only validates and matches orders on arrival. The same is true of the world layer (`world.py`), where an empire action is validated JSON just like an order; it holds the map, the buildings, the units and the combat, and is kept pure and seeded so the whole ruleset is testable without standing up a market. A player's trading P&L is their build budget, which is the only coupling between the two halves. Both keep their own books and fictional items, separate from the live stock market. Full write-up — architecture, the API, and a strategy-writing guide with worked examples — in **[docs/market_sim_py.md](docs/market_sim_py.md)**.
